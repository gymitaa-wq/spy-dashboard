...
# --- 页面标题 ---
st.title("📈 Put Option Selling Dashboard")
...
def display_ticker_dashboard(ticker_symbol, min_dte, max_dte):
    ...
    Args:
        ...
        max_dte: 最大到期天数
    """
    if st.button(f'刷新数据 (Fetch Data) - {ticker_symbol}',
                 key=f"btn_{ticker_symbol}"):
        with st.spinner(f'正在从 Yahoo Finance 获取 {ticker_symbol} 数据...'):
            df, spot_price, error_msg = get_option_data(
                ticker_symbol, min_dte, max_dte)

            if error_msg:
                st.error(error_msg)
            elif spot_price:
                st.metric(label=f"{ticker_symbol} Current Price",
            ...
            if df is not None and not df.empty:
                ...
                df_display = df_display.sort_values(
                    by=['dte', 'moneyness'], ascending=[True, True])
                ...
                st.download_button(
                    f"下载 {ticker_symbol} CSV",
                    csv,
                    f"{ticker_symbol}_puts_filtered.csv",
                    "text/csv",
                    key=f"dl_{ticker_symbol}"
                )
